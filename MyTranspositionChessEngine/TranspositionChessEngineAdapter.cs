...
{
    /// Adapts the TranspositionChessEngine to the IChessEngine interface used by the UI:
    /// translates positions, moves, pieces and ratings between the base types
    /// and the integer representation.
    public class TranspositionChessEngineAdapter : IChessEngine
    {
        private readonly TranspositionChessEngine Engine = new();
        ...
        public BaseMove CalculateMove(int depth)
        {
            ...
            if (move == null)
            {
                _Message = "TranspositionChessEngine: no move possible. " + Engine.GetRating();
                return BaseMove.CreateNoMove(ToBoardRating(Engine.GetRating()));
            }

            TranspositionTable? table = Engine.Board.Table;
            string tableStatistics = table == null
                ? ""
                : Environment.NewLine + $"TT: {table.Hits} hits / {table.Probes} probes, {table.Stores} stores";
