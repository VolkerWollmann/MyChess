...
namespace MyTranspositionChessEngine
{
    ...
    public class Board
    {
        ...
        /// Transposition table of the running search. Created fresh per
        /// CalculateMove/CalculateMoveParallel call and shared with the board
        /// copies of the parallel root search; entries are keyed by position
        /// and remaining depth, so all workers may read and write it.
        public TranspositionTable? Table { get; private set; }

        /// Zobrist key of the current position: pieces, castle rights, pawns
        /// capturable en passant right now, and the side to move - exactly the
        /// state move generation and evaluation depend on. Computed by a full
        /// scan; cheap against the move generation a table hit saves.
        internal ulong ComputeZobristKey()
        {
            ulong key = 0;
            ...
            {
                int pieceValue = Field[Constants.BroadPlane, column, row];
                if (pieceValue == Constants.NoPiece)
                    continue;

                int pieceIndex = (pieceValue & Constants.PieceMask) - 1
                                 + ((pieceValue & Constants.ColorMask) == Constants.Black ? 6 : 0);
                int square = (column - 2) * ChessEngineConstants.Length + (row - 2);
                key ^= Zobrist.PieceKeys[pieceIndex, square];

                // A pawn gets an en passant key exactly when Pawn.GetMoveList
                // would offer the capture: fresh marking (marking + 1 ==
                // CurrentPly, only on the immediate reply) plus an adjacent
                // enemy pawn and an empty capture square. Mirroring the full
                // condition keeps the key identical for positions in which no
                // en passant move exists either way.
                if ((pieceValue & Constants.PieceMask) == Constants.Pawn
                    && Field[Constants.EnPassantPlane, column, row] + 1 == CurrentPly
                    && EnPassantCapturePossible(column, row, pieceValue & Constants.ColorMask))
                    key ^= Zobrist.EnPassantKeys[square];
            }

            if (WhiteCastleKingSidePossible())
                key ^= Zobrist.CastleKeys[0];
            if (WhiteCastleQueenSidePossible())
                key ^= Zobrist.CastleKeys[1];
            if (BlackCastleKingSidePossible())
                key ^= Zobrist.CastleKeys[2];
            if (BlackCastleQueenSidePossible())
                key ^= Zobrist.CastleKeys[3];

            if (ColorToMove == Constants.Black)
                key ^= Zobrist.BlackToMoveKey;

            return key;
        }

        /// True if the marked pawn (padded grid coordinates) could capture en
        /// passant right now: an adjacent enemy pawn and an empty forward
        /// diagonal behind it - the same checks as TryAdd*EnPassant.
        private bool EnPassantCapturePossible(int column, int row, int color)
        {
            int forward = color == Constants.White ? 1 : -1;
            int enemyPawn = Constants.Pawn | (color == Constants.White ? Constants.Black : Constants.White);

            if (Field[Constants.BroadPlane, column - 1, row] == enemyPawn
                && Field[Constants.BroadPlane, column - 1, row + forward] == Constants.NoPiece)
                return true;

            return Field[Constants.BroadPlane, column + 1, row] == enemyPawn
                   && Field[Constants.BroadPlane, column + 1, row + forward] == Constants.NoPiece;
        }
        ...
        private (Move? Move, Rating Rating) Search(int depth, int alpha, int beta)
        {
            ...
            // Transposition table, only for inner nodes (depth >= 1): the same
            // position reached by a different move order reuses the stored
            // result instead of searching the subtree again. Leaf evaluations
            // stay uncached - their threat-field term reuses the parent's
            // marking (one ply stale), so the same position can evaluate
            // differently on different paths; from depth 1 up every value is a
            // pure function of position and remaining depth.
            ulong key = ComputeZobristKey();

            TranspositionEntry? entry = Table?.Probe(key, depth);
            if (entry != null)
            {
                switch (entry.Bound)
                {
                    case Bound.Exact:
                        return (null, entry.ToRating());
            ...
            // Window the node was actually searched with, for the bound
            // classification on store (fail-soft alpha-beta).
            ...
            if (bestMove == null)
            {
                ...
                Rating finalRating = IsKingThreatened(ColorToMove)
                    ? (white
                        ? new Rating(-Constants.KingValue - depth * Constants.WinDepthValue, GameState.WhiteLoss)
                        : new Rating(Constants.KingValue + depth * Constants.WinDepthValue, GameState.BlackLoss))
                    : new Rating(0, GameState.Remis);

                StoreInTable(key, depth, finalRating, alphaBefore, betaBefore);
                return (null, finalRating);
            }
            ...
            StoreInTable(key, depth, bestRating, alphaBefore, betaBefore);

            return (bestMove, bestRating)!;
        }

        /// Fail-soft bound classification: a value outside the search window is
        /// only a bound on the true value (children were cut off or failed low),
        /// inside the window it is exact. Exact entries answer a later probe
        /// directly, bounds only narrow its window.
        private void StoreInTable(ulong key, int depth, Rating rating, int alphaBefore, int betaBefore)
        {
            if (Table == null)
                return;

            Bound bound = Bound.Exact;
            if (rating.Value <= alphaBefore)
                bound = Bound.Upper;
            else if (rating.Value >= betaBefore)
                bound = Bound.Lower;

            Table.Store(key, depth, rating.Value, bound, rating.State, rating.MoveList);
        }
        ...
        public Board Copy()
        {
            ...
            // The copies of the parallel root search share one table; entries
            // are immutable and keyed by position + depth, so every worker may
            // read results the others stored.
            newBoard.Table = Table;
